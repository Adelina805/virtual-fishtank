...
export function modeHudPresetChip(isNight: boolean, selected: boolean): string {
  ...
  const base = `min-w-[2rem] rounded-lg px-2 py-1 text-center text-[0.7rem] font-semibold tabular-nums focus-visible:outline-2 focus-visible:outline-offset-2 ${focusRing}`;
  if (selected) {
    return isNight
